...
typedef struct _APPLE_BACKUP_KEY {
	DWORD version;
	DWORD keyLen;
	DWORD certLen;
	BYTE data[ANYSIZE_ARRAY];
} APPLE_BACKUP_KEY, *PAPPLE_BACKUP_KEY;
...
typedef struct _APPLE_ENC_SC_DATA {
	BYTE toSign[32];
	BYTE toHash[32];
	BYTE toDecrypt[ANYSIZE_ARRAY];
} APPLE_ENC_SC_DATA, *PAPPLE_ENC_SC_DATA;

typedef struct _APPLE_ENC_SC_DATA_NEW {
	BYTE Header[8]; // SuppData
	DWORD unk0;
	DWORD unk1;
	DWORD unk2;
	DWORD dataSize;
	APPLE_ENC_SC_DATA data;
} APPLE_ENC_SC_DATA_NEW, *PAPPLE_ENC_SC_DATA_NEW;
...
typedef struct _APPLE_LSA_PRIVATE_DATA {
	DWORD DataType;
	WORD LmLength;
	...
	// NtHistoryArray
	// LmHistoryArray
} APPLE_LSA_PRIVATE_DATA, *PAPPLE_LSA_PRIVATE_DATA;
...
typedef struct _APPLE_TBAL_MSV {
	DWORD unk0;
	DWORD structLen;
	DWORD flags;
	DWORD unkD; // why not ?
...
} APPLE_TBAL_MSV, *PAPPLE_TBAL_MSV;

typedef struct _APPLE_ENCRYPTED_SUPPLEMENTAL_CREDENTIALS {
	DWORD unk0;
	DWORD unkSize;
	DWORD unk1; // flags ?
	DWORD originalSize;
...
} APPLE_ENCRYPTED_SUPPLEMENTAL_CREDENTIALS, *PAPPLE_ENCRYPTED_SUPPLEMENTAL_CREDENTIALS;
